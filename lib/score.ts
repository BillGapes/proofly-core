import { ratings, standouts } from './mock';
...
export function computeProoflyScore(userId: string): number {
  ...
  const now = Date.now();
  const halfLife = 1000 * 60 * 60 * 24 * 540; // ~18 months
  ...
  for (const r of my) {
    ...
    starDen += 5 * decay * rel;
    freshBoost += decay * 4; // recent activity bonus
  }
  const starPct = starDen ? (starNum / starDen) : 0;          // 0..1
  const volume = Math.min(1, my.length / 15);                 // saturate ~15
